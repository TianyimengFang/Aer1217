...
class ROSLabInterface(object):
    ...
    def __init__(self):
        ...
        # Publishers
        self.pub_vel_prop = rospy.Publisher('/aer1217_ardrone/vel_prop', MotorCommands, queue_size=300)

        self.model_name = 'ARDroneCarre'
        
        self.pub_vicon_data = rospy.Publisher('/vicon/{0}/{0}'.format(self.model_name), TransformStamped, queue_size=30)
        
        # Subscribers
        self.sub_gazebo_pose = rospy.Subscriber('/aer1217_ardrone/gazebo_state', GazeboState, self.update_quadrotor_state)
        
        self.sub_cmd_vel = rospy.Subscriber('cmd_vel_RHC', Twist, self.update_offboard_command)
        ...
        self.pub_prop_vel = rospy.Timer(rospy.Duration(1. / self.onboard_loop_frequency), self.update_motor_speeds)
...
class ARDroneOnboardController(object):
	...
	#subscribers for takeoff and landing 
        self.sub_land = rospy.Subscriber('/ardrone/land', Empty, self.land)
        self.sub_takeoff = rospy.Subscriber('/ardrone/takeoff', Empty, self.takeoff)
	###
